...
import { Home, Radar, BarChart3, GraduationCap, Trophy, Settings as SettingsIcon, LogOut } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="flex min-h-screen">
        <aside className="hidden md:flex flex-col w-64 border-r border-cyber-border bg-cyber-bg/60 backdrop-blur-md p-4 fixed h-full">
          ...
          <nav className="flex-1 mt-4 space-y-1">
            {TABS.map((tab) => (
              <NavLink key={tab.to} to={tab.to} end={tab.end} className={({ isActive }) => `flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all ${isActive ? 'gradient-primary text-white glow-primary' : 'text-slate-400 hover:text-white hover:bg-cyber-surface/40'}`}>
        ...
        <nav className="md:hidden fixed bottom-0 left-0 right-0 z-30 bg-cyber-bg/90 backdrop-blur-md border-t border-cyber-border flex items-center justify-around py-2">
          {TABS.map((tab) => (
            <NavLink key={tab.to} to={tab.to} end={tab.end} className={({ isActive }) => `flex flex-col items-center gap-1 px-2 py-1 rounded-lg transition-colors ${isActive ? 'text-cyber-glow' : 'text-slate-500'}`}>
              <tab.icon size={20} /><span className="text-[10px] font-semibold">{tab.label}</span>
            </NavLink>
          ))}
        </nav>
      </div>
