...
export default function Home() {
  ...
  const stats = [
    ...
    { label: 'Hosts Scanned', value: progress.scannedHosts.length, icon: Radar },
  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center gap-3 pt-4">
        ...
        <p className="text-sm text-slate-400 text-center px-4">Real-Time Network Port Scanner</p>
      </motion.div>
      ...
      {progress.lastScan ? (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="glass-card p-5">
          ...
          <div className="flex items-center justify-between">
            <div><p className="text-lg font-extrabold text-white">{progress.lastScan.host}</p><p className="text-xs text-slate-400 mt-1">{progress.lastScan.openCount} open · {progress.lastScan.closedCount} closed · {formatDuration(progress.lastScan.durationMs)}</p></div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="glass-card p-5">
          <div className="flex items-center gap-2 mb-3"><GraduationCap size={18} className="text-cyber-accentGlow" /><h2 className="text-base font-bold text-white">Start Scanning</h2></div>
          <p className="text-sm text-slate-400 leading-relaxed">Run your first real TCP port scan to see live results, statistics, and security insights.</p>
          <button onClick={() => navigate('/scanner')} className="mt-4 flex items-center gap-1.5 text-sm font-bold text-cyber-glow border border-cyber-border rounded-xl px-4 py-2.5 hover:bg-cyber-surface/30 transition-colors">Go to Scanner <ChevronRight size={16} /></button>
        </motion.div>
      )}
